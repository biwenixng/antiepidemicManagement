...
/**
 * Servlet implementation class UpdateHear
 */
@Controller
...
public class UpdateHead extends HttpServlet {
	...
	protected String updateHead(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
				...
				for (FileItem item : items) {
					...
					if (item.isFormField()) {
						...
						if (!value) {
							...
							return "personalCenter/update_head";
						}
						// 1.WEB-INF 2.目录打散 3.文件重名
						
										
						String descPath = realPath + "/personalCenter/images/head_images";
...
//						File file = new File("D:\\web服务器\\xiangmu\\WebContent/personalCenter/images/head_images",name);
						//System.out.println(file);
						String headimg= "/personalCenter/images/head_images/"+ name;
						
						String username = request.getParameter("username");
						
						service.updateHead(headimg, username);
						item.write(fastPath);
//						item.write(file);
				...
				request.getRequestDispatcher("personalCenter/upload_code.jsp").forward(request, response);
