			...
			<div class="table-responsive">
							...
							<c:if test="${not empty page.pages}">
								...
								<c:forEach var="item" items="${page.pages}">
									<tr>
										...
										<td>${item.vaildty}</td>
										<td> 
											<button type="button" data-toggle="modal" data-target="#exampleModal" cid="${item.cid}">修改</button>
										</td>
									</tr>
								</c:forEach>
							</c:if>
				...
				<ul class="pagination">
					<c:if test="${page.currentPage != 1}">
				        <li>
				            <a href="<c:url value='/doctor/doctorServlet?method=showCurrentPage&currentPage=${page.currentPage - 1}&pageTotal=1&transmit=alter_commodity2.jsp'/>" aria-label="Previous">
			        ...
			        <c:forEach var="index" begin="${start}" end="${end}">
			        	<c:choose>
			        		<c:when test="${index == page.currentPage}">
			        			<li>
					        		<a style="background: blue;color: red;" href='<c:url value='/doctor/doctorServlet?method=showCurrentPage&currentPage=${index}&pageTotal=1&transmit=alter_commodity2.jsp'/>'>${index}</a>
					        	</li>
			        		</c:when>
			        		<c:otherwise>
			        			<li>
					        		<a href='<c:url value='/doctor/doctorServlet?method=showCurrentPage&currentPage=${index}&pageTotal=1&transmit=alter_commodity2.jsp'/>'>${index}</a>
					        	</li>
			        		</c:otherwise>
			        	</c:choose>
			        </c:forEach>
			        <c:if test="${page.currentPage != page.pageCount}">
				        <li id="end">
				            <a href="<c:url value='/doctor/doctorServlet?method=showCurrentPage&currentPage=${page.currentPage + 1}&pageTotal=1&transmit=alter_commodity2.jsp'/>" aria-label="Next">
