	...
	<div style="width: 97%;">
		        		...
		        		<c:forEach var="item" items="${page.pages}">
		        			<tr>
		        				...
		        				<c:choose>
		        					...
		        					<td><a class="btn btn-default" href='<c:url value="/admin/adminServlet?method=removeNews&currentPage=${page.currentPage}&pageTotal=1&show=0&pid=${item.pid}"/>' role="button">删除</a></td>
		        					</c:when>
		        					<c:otherwise>
		        					<td><a class="btn btn-default" href='<c:url value="/admin/adminServlet?method=removeNews&currentPage=${page.currentPage}&pageTotal=1&show=1&pid=${item.pid}"/>' role="button">恢复</a></td>
		        					</c:otherwise>
		        				</c:choose>
		        			</tr>
		        		</c:forEach>
		...
		<ul class="pagination">
			   <c:choose>
			    	...
			    	<c:otherwise>
			    		<li>
				    		<a href='<c:url value='/admin/adminServlet?method=newsPageAll&currentPage=${page.currentPage - 1}&pageTotal=1'/>' aria-label='Previous'>
			    ...
			    <c:forEach var="item" begin="${start}" end="${end}">
			    	<c:choose>
			    		<c:when test="${page.currentPage == item}">
				    		<li class='active'>
						    	<a href='<c:url value='/admin/adminServlet?method=newsPageAll&currentPage=${item}&pageTotal=1'/>'>
						    		${item}
						    	</a>
					    	</li>
			    		</c:when>
			    ...
			    <c:choose>
			    	...
			    	<c:otherwise>
			    		<li>
				    		<a href='<c:url value='/admin/adminServlet?method=newsPageAll&currentPage=${page.currentPage + 1}&pageTotal=1'/>' aria-label='Next'>
